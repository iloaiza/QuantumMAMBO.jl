"""
Targeted coeffcients gate, adapted from cirq-ft's StatePreparationAliasSampling
"""
...
@cirq.value_equality()
@attr.frozen
class TargetedCoefficients(select_and_prepare.PrepareOracle):
    ...
    @classmethod
    def from_lcu_probs(
        cls, lcu_probabilities: List[float], *, probability_epsilon: float = 1.0e-5, sel_reg = None
    ) -> 'TargetedCoefficients':
        ...
        if type(sel_reg) == type(None):
            sel_reg = infra.SelectionRegister('selection', (N - 1).bit_length(), N)

        return TargetedCoefficients(
            selection_registers=infra.SelectionRegisters(
                [sel_reg]
            ),
            alt=np.array(alt),
    ...
    def decompose_from_registers(
        ...
        **quregs: NDArray[cirq.Qid],  # type:ignore[type-var]
        ...
        assert len(self.selection_registers) == 1
